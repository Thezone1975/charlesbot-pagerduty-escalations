...
help:
	@echo '-----------------------------------------'
	@echo ' Pagerduty Escalations make targets'
	@echo '-----------------------------------------'
	@echo 'help: This help'
...
checkstyle: install
	$(ENV)/bin/flake8 --max-complexity 10 charlesbot_pagerduty_escalations
...
test: install
	$(ENV)/bin/nosetests \
		-v \
		--with-coverage \
		--cover-package=charlesbot_pagerduty_escalations \
		tests
